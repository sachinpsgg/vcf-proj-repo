...
const GeneratedURLs = () => {
  ...
  const uniqueCampaigns = Array.from(
    new Set(generatedUrls.map((url) => url.campaignName)),
  ).map((name) => ({
    id:
      generatedUrls.find((url) => url.campaignName === name)?.campaignId || "",
    name,
  }));
  ...
  const getStatusStats = () => {
    ...
    const active = generatedUrls.filter((url) => url.isActive).length;
