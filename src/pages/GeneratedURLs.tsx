...
const GeneratedURLs = () => {
  ...
  const uniqueCampaigns = campaignId
    ? []
    : Array.from(new Set(generatedUrls.map((url) => url.campaignName))).map((name) => ({
      id: generatedUrls.find((url) => url.campaignName === name)?.campaignId || "",
      name,
    }));
  ...
  const handleToggleStatus = (urlId: string) => {
    setGeneratedUrls((urls) =>
      urls.map((url) => (url.id === urlId ? { ...url, isActive: !url.isActive } : url))
    );
    toast.success("URL status updated!");
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.total}</div>
              <p className="text-xs text-muted-foreground">Contact cards created</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`grid grid-cols-1 gap-4 ${campaignId ? "md:grid-cols-3" : "md:grid-cols-4"}`}>
              <div className="space-y-2">
                <label className="text-sm font-medium">Search</label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground w-4 h-4" />
                ...
                {filteredUrls.length === 0 ? (
                  ...
                  filteredUrls.map((urlData) => (
                    <TableRow key={urlData.id}>
                      <TableCell>
                        <div>
                          <div className="font-medium">{urlData.doctorName}</div>
                          <div className="text-sm text-muted-foreground font-mono">{urlData.doctorPhone}</div>
                        </div>
                      </TableCell>
                      {!campaignId && (
                        <TableCell>
                          <div>{urlData.campaignName}</div>
                        </TableCell>
                      )}
                      <TableCell>
                        <div className="flex items-center space-x-2">
