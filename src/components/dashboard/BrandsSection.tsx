...
const BrandsSection = () => {
  ...
  const fetchBrands = async () => {
    try {
      ...
      console.error("Error fetching brands:", error);
      toast.error("Failed to refresh brands list");
    }
  };

  const handleCreateBrand = (brandData: Omit<Brand, "id" | "createdAt">) => {
    // Refresh the brands list from the server to get the latest data
    fetchBrands();
  };
  ...
  const handleEditBrand = (
    ...
    // Refresh the brands list from the server instead of local state update
    // This ensures we get the latest data and avoid state conflicts
    fetchBrands();
  };
  useEffect(() => {
    fetchBrands();
